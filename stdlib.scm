(define map
 ...
 (define fold-left 
	(let ((car car) (cdr cdr) (null? null?) (= =) (+ +) (list? list?) (eq? eq?))
		(letrec ((lefthelper (lambda (f acc list)
							(if (null? list)
								acc
								(lefthelper f (f acc (car list)) (cdr list))))))
		(lambda (f acc list) (lefthelper f acc list)))
	))


(define fold-right
	(let ((car car) (cdr cdr) (null? null?) (+ +) (= =) (list? list?) (eq? eq?))
		(letrec ((righthelperc (lambda (f acc list)
							(if (null? list)
								acc
								(f (car list) (righthelperc f acc (cdr list)))))))
		(lambda (f acc list) (righthelperc f acc list)))
	))



(define last-ele
	(let ((car car) (cdr cdr) (null? null?) (list? list?) (cons cons) (+ +) (= =) (eq? eq?))
		(letrec ((helper (lambda (ob) (if (null? (cdr ob))
										(car ob)
										(helper (cdr ob))))))
		(lambda (ob) (helper ob)))
))
...
(define check-list
	(let ((car car) (cdr cdr) (null? null?) (list? list?) (cons cons) (+ +) (= =) (eq? eq?))
		(letrec ((helper (lambda (ob fin) (if (eq? (car ob) fin)
										fin
										(cons (car ob)
										(helper (cdr ob) fin))))))
		(lambda (ob fin) (helper ob fin)))
))


(define cons*
  (let ((car car) (cdr cdr) (null? null?) (list? list?) (cons cons) (+ +) (= =) (eq? eq?))
    (letrec ((cons-star (lambda (first-obj . objects) (if(null? objects)
            											first-obj
            											(check-list (cons first-obj objects) (last-ele objects))))))
    cons-star)))
